...
def main():
    ...
    """)
    try:
        prob_type = int(input("¿Que opcion desea?: >"))
    except ValueError:
    ...
    global const_names
    const_num = int(input("¿Cuantas tipos de semilla de cafe utilizara?: >"))
    prod_nums = int(input("Cuantas Restricciones se compone el problema: >"))
    const_names = [x + str(i) for i in range(1, const_num + 1)]
    for i in range(1, prod_nums + 1):
        prod_val = input("Introdusca el nombre de la caracteristica de la restriccion {} : >".format(i))
        product_names.append(prod_val)
    print("__________________________________________________")
    if prob_type == 1:
        for i in const_names:
            try:
                val = float(Fraction(input("Introdusca las utilidades para la semilla  %s de la ecuacion Z: >" % i)))
            except ValueError:
        ...
        for prod in product_names:
            for const in const_names:
                try:
                    val = float(Fraction(input("Introdusca el valor  para la semilla  %s en %s: >" % (const, prod))))
                except ValueError:
                    print("please ensure you enter a number")
                    val = float(Fraction(input("enter the value of %s in %s: >" % (const, prod))))
                col_values.append(val)
            equate_prod = float(Fraction(input('maximo de %s : >' % prod)))
        ...
        decimals = int(input('Numero de decimales que manejaremos : '))
        print('\n##########################################')
